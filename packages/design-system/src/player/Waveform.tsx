...
export interface WaveformProps {
  ...
  seed?: string;
  /** Real normalised 0-1 peaks generated from the audio file. */
...
export const Waveform: React.FC<WaveformProps> = ({
  ...
  return (
    <div
      ...
      className={`group relative flex h-7 sm:h-9 w-full items-end gap-[1.5px] sm:gap-[2px] py-1 cursor-pointer select-none transition-all duration-300 ${
      ...
      <AnimatePresence>
        {hoverPercent !== null && onSeek && (
          <motion.div
            ...
            className="pointer-events-none absolute -top-1 -translate-x-1/2 z-30 flex items-center gap-1 rounded-full border border-[#e59e38]/40 bg-black/90 px-2.5 py-0.5 shadow-[0_4px_20px_rgba(0,0,0,0.8),0_0_12px_rgba(229,158,56,0.4)] backdrop-blur-md"
          >
            <span className="font-mono text-[10px] font-bold text-[#e59e38] tabular-nums">
      ...
      {heights.map((h, i) => {
        ...
        return (
          <div
            key={i}
            style={{ height: `${h}%` }}
            className={`relative flex-1 min-w-[1px] rounded-full transition-all duration-150 ease-out origin-bottom ${
              played
                ? "bg-gradient-to-t from-[#e59e38] via-amber-400 to-amber-300 shadow-[0_0_8px_rgba(229,158,56,0.4)]"
                : isHovered
                ? "bg-white/40"
          ...
          >
            ...
            {played && i === Math.floor((progress / 100) * bars) && (
              <span className="absolute -top-1 left-1/2 -translate-x-1/2 h-2 w-2 rounded-full bg-white shadow-[0_0_10px_#e59e38,0_0_20px_#e59e38] animate-pulse" />
            )}
          </div>
        );
      })}
