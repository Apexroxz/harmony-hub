...
/**
 * The Cycle (Avartan) Brand SVG Mark
 * 8 beat-points representing the rhythmic cycle in Hindustani taal (7 solid, 1 hollow khali).
 */
export const CycleBrandMark: React.FC<{ className?: string }> = ({ className = "h-7 w-7" }) => (
  <svg
    viewBox="0 0 100 100"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={`${className} select-none shrink-0 transition-transform duration-150 ease-out`}
    aria-hidden="true"
  >
    <circle cx="50" cy="20" r="6" fill="#e59e38" />
    <circle cx="71.2" cy="28.8" r="6" fill="#e59e38" />
    <circle cx="80" cy="50" r="6" fill="#e59e38" />
    <circle cx="71.2" cy="71.2" r="6" fill="#e59e38" />
    <circle cx="50" cy="80" r="6" fill="none" stroke="#e59e38" strokeWidth="3.5" />
    <circle cx="28.8" cy="71.2" r="6" fill="#e59e38" />
    <circle cx="20" cy="50" r="6" fill="#e59e38" />
    <circle cx="28.8" cy="28.8" r="6" fill="#e59e38" />
  </svg>
);
...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  if (variant === "mark") {
    return (
      ...
      >
        <CycleBrandMark className={currentSize.mark} />
      </div>
    );
  }
    ...
    return (
      ...
      >
        ...
        <div className="flex items-baseline gap-1.5">
          ...
          {showBadge && (
            <span className={`${currentSize.badge} font-tech uppercase font-bold text-[#e59e38] bg-[#e59e38]/10 border border-[#e59e38]/25 rounded-[6px]`}>
              {badgeText}
            </span>
          )}
        </div>
    ...
    >
      ...
      <div className="flex flex-col justify-center min-w-0">
        <div className="flex items-center gap-2">
          ...
          {showBadge && (
            <span className={`${currentSize.badge} font-tech uppercase font-semibold text-[#e59e38] bg-[#e59e38]/10 border border-[#e59e38]/25 rounded-[6px] hidden sm:inline-block`}>
              {badgeText}
            </span>
          )}
        </div>
