...
export class OfflineService {
  ...
  public static async importFiles(files: FileList | File[]): Promise<LocalTrack[]> {
    ...
    for (let i = 0; i < fileArray.length; i++) {
      ...
      try {
        ...
        console.warn("[OfflineService] IndexedDB storage fallback:", err);
        blobUrl = URL.createObjectURL(file);
      }
      ...
      const localTrack: LocalTrack = {
        ...
        artistName: meta.artist || "Local Artist",
  ...
  public static async saveTrackOffline(track: Track): Promise<void> {
    ...
    let blobUrl = track.audioUrl || "";
    try {
      if (track.audioUrl && !track.audioUrl.startsWith("blob:")) {
        const res = await fetch(track.audioUrl);
        const blob = await res.blob();
        await storeAudioBlob(track.id, blob);
    ...
    const localTrack: LocalTrack = {
      ...track,
      id: track.id.startsWith("local-") ? track.id : `local-${track.id}`,
      source: "offline",
