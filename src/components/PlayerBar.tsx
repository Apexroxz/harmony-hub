...
export function PlayerBar() {
  ...
  const sampleRateLabel = currentTrack.sampleRate
  ...
  return (
    <>
      ...
      {/* ── Slide-up Audiophile Queue Popover ── */}
      <AnimatePresence>
        {queueOpen && (
          <motion.aside
            ...
            className="fixed bottom-[110px] right-4 z-50 max-h-[56vh] w-[min(92vw,26rem)] overflow-hidden rounded-2xl border border-[#D99A2B]/25 bg-[#08090B] shadow-[0_20px_60px_rgba(0,0,0,0.95)] backdrop-blur-3xl sm:right-8 flex flex-col"
          >
            ...
            <ul className="max-h-[40vh] overflow-y-auto px-3 pb-3 space-y-1">
              {queue.length === 0 && (
      ...
      {/* ── High-End Audiophile Hardware Cockpit Chassis ── */}
      <div
        onDragOver={handleDragOver}
        onDragLeave={handleDragLeave}
        onDrop={handleDrop}
        className={cn(
          "fixed bottom-6 left-4 right-4 z-40 max-w-6xl mx-auto rounded-2xl bg-[#08090B] border border-[#D99A2B]/20 shadow-[0_20px_80px_rgba(0,0,0,0.85)] p-3 sm:px-5 sm:py-3.5 transition-all duration-200 backdrop-blur-none",
          isDraggingOver
      ...
      >
        ...
        <div className="flex flex-col gap-2.5">
          {/* Main Cockpit Row: [ Cartridge + Info ] · [ Precision Waveform & Transport ] · [ Hardware Utilities ] */}
